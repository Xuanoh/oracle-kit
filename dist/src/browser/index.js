...
import { ensureChatGptSelection } from "./actions/chatGptSelection.js";
...
export async function runBrowserMode(options) {
    ...
    try {
        ...
        if (config.selectionPlan) {
            modelSelectionEvidence = await raceWithDisconnect(ensureChatGptSelection(Runtime, config.selectionPlan, logger, { observeOnly: isResumingConversation }));
            await raceWithDisconnect(ensurePromptReady(Runtime, config.inputTimeoutMs, logger));
        }
        else if (config.desiredModel && modelStrategy !== "ignore" && !isResumingConversation) {
        ...
        if (thinkingTime && !deepResearch && !config.selectionPlan) {
...
async function runRemoteBrowserMode(promptText, attachments, config, logger, options) {
    ...
    try {
        ...
        const modelStrategy = config.modelStrategy ?? DEFAULT_MODEL_STRATEGY;
        if (config.selectionPlan) {
            modelSelectionEvidence = await ensureChatGptSelection(Runtime, config.selectionPlan, logger, { observeOnly: Boolean(config.resumeConversationUrl) });
            await ensurePromptReady(Runtime, config.inputTimeoutMs, logger);
        }
        else if (config.desiredModel && modelStrategy !== "ignore" && !config.resumeConversationUrl) {
