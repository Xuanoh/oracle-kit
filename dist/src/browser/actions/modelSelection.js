...
export function assertResolvedModelSelection(desiredModel, resolvedLabel) {
    ...
    if (normalizedDesired === "gpt 6 astra") {
        if (!/(?:^| )gpt 6 astra(?: |$)/.test(normalizedResolved) || normalizedResolved.split(" ").includes("pro")) {
            throw new Error(`Model picker selected "${resolvedLabel}" while "${desiredModel}" requires GPT-6 Astra.`);
        }
        return;
    }
...
function buildModelSelectionExpression(targetModel, strategy) {
    ...
    const desiredVersion = normalizedTarget === 'gpt 6 astra'
      ? '6'
      ...
      : normalizedTarget.includes('5 5')
        ...
        : normalizedTarget.includes('5 1')
          ...
          : normalizedTarget.includes('5 0')
            ? '5-0'
          : null;
    const desiredModelVariant = normalizedTarget.includes(' astra') ? 'astra' : normalizedTarget.includes(' sol') ? 'sol' : null;
    ...
    const versionFromLabel = (label) => {
      if (normalizeText(label).split(' ').includes('6') && !normalizeText(label).includes('5 6')) return '6';
      const match = normalizeText(label).match(/(?:^| )(?:gpt )?(\\d+) (\\d+)(?: |$)/);
      return match ? match[1] + '-' + match[2] : null;
    };
    const versionFromTestId = (testid) => {
      const normalized = normalizeText(testid);
      if (normalized.split(' ').includes('6') && !normalized.includes('5 6')) return '6';
    ...
    const configuredSelectionMatchesTarget = () => {
      ...
      if (desiredModelVariant && labelHasProWord(configuredVariant)) return false;
    ...
    const buttonMatchesTarget = () => {
      ...
      if (desiredModelVariant && hasProComposerPill()) return false;
    ...
    const composerSignalMatchesTarget = () => {
      ...
      if (desiredModelVariant && hasProComposerPill()) {
        return false;
      }
    ...
    const scoreOption = (normalizedText, testid, node) => {
      ...
      if (desiredModelVariant && candidateHasPro) return 0;
    ...
    const canTrustSelectedOption = (node, normalizedText, testid) => {
      ...
      if (desiredVersion === '5-6' || desiredVersion === '6') return !hasProComposerPill();
      const currentButtonLabel = normalizeText(getButtonLabel());
...
function buildComposerSignalMatchers(targetModel) {
    ...
    if (normalized === "gpt 6 astra") {
        return { includesAny: ["gpt 6 astra"], excludesAny: ["pro"], allowBlank: false };
    }
