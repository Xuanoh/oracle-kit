...
export async function runBrowserPreview({ runOptions, cwd, version, previewMode, log, browserConfig, }, deps = {}) {
    ...
    log(chalk.cyan(headerLine));
    log(`[preview] Surface: ${browserConfig?.surface ?? "chat"}; model menu: ${browserConfig?.desiredModel ?? runOptions.model}; thinking time: ${browserConfig?.thinkingTime ?? "current setting (not selected)"}.`);
    ...
    if (previewMode === "json" || previewMode === "full") {
        ...
        const previewPayload = {
            ...
            engine: "browser",
            surface: browserConfig?.surface ?? "chat",
            selectionPlan: browserConfig?.selectionPlan ?? null,
            desiredModel: browserConfig?.desiredModel ?? runOptions.model,
            thinkingTime: browserConfig?.thinkingTime ?? null,
